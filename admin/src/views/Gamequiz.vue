<template>
 <div>
   <h1>gioco 1</h1>
   <div v-if="domande[0]!=null" class="block py-8 px-8 max-w-sm mx-auto bg-white rounded-xl shadow-md space-y-2 sm:py-4 sm:flex flex-col sm:items-center sm:space-y-0 sm:space-x-2">
     <div v-if="numerodomanda<lunghezza" class="sm:flex flex-col sm:items-center">
     <h1>{{this.domande[numerodomanda].domanda}}</h1>
<button  @click="risposta(index)"  v-for="(item,index) in domande[numerodomanda].argomento" :key="index" class="px-4 py-1 text-sm text-purple-600 font-semibold rounded-full border border-purple-200 hover:text-white hover:bg-purple-600 hover:border-transparent focus:outline-none focus:ring-2 focus:ring-purple-600 focus:ring-offset-2">
   {{item}}
   </button>
   <button v-if="numerodomanda<lunghezza" @click="avanti">
     avanti
   </button>
 </div>
