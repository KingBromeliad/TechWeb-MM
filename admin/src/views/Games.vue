<template>
  <div>
  <div class="block py-8 px-8 max-w-sm mx-auto bg-white rounded-xl shadow-md space-y-2 sm:py-4 sm:flex flex-col sm:items-center sm:space-y-0 sm:space-x-2">
    <div>
      <div v-if="parolachiave[0]!=null" class="py-8">
         <button @click="rimuovi(i)" v-for="(item, i) in parolachiave" :key="i" v-bind:id="'A'+i" class="px-4 py-1 text-sm text-purple-600 font-semibold rounded-full border border-purple-200 hover:text-white hover:bg-purple-600 hover:border-transparent focus:outline-none focus:ring-2 focus:ring-purple-600 focus:ring-offset-2">
            {{item}}
          </button>
        </div>
      <div v-if="parolachiave2[0]!=null">
        <button @click="inserisci(index)" v-for="(item2,index)  in parolachiave2" :key="index" v-bind:id="'button2'+index" class="px-4 py-1 text-sm text-purple-600 font-semibold rounded-full border border-purple-200 hover:text-white hover:bg-purple-600 hover:border-transparent focus:outline-none focus:ring-2 focus:ring-purple-600 focus:ring-offset-2">
          {{item2}}
        </button>
          </div>
        </div>
        <div>
          ...
          <button @click="avanti()" class="px-4 py-1 text-sm text-purple-600 font-semibold rounded-full border border-purple-200 hover:text-white hover:bg-purple-600 hover:border-transparent focus:outline-none focus:ring-2 focus:ring-purple-600 focus:ring-offset-2"> avanti</button>
           </div>
      <div v-else>
        <h1>HAI VINTO</h1>
